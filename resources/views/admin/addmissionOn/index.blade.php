...
@section('title','Admission On')
@section('page-header')
    <i class="fa fa-list"></i> Addmission Going On
@stop
@section('css')
...
@section('content')
    ...
    <div class="page-header">
        ...
        <a class="btn btn-xs btn-info" href="{{ route('addmissionOn.create') }}" role="button" class="blue" data-toggle="modal" style="float: right; margin: 0 2px;"> <i class="fa fa-plus"></i> Add New</a>
    ...
    <div class="row">
     ...
     <td>
      ...
      <img class="img-responsive img-thumbnail" src="{{ asset('uploads/addmissionOn/'.$item->image) }}"  style="height: 50px; weight: 30px;" alt=""> 
      ...
      @endif
             ...
             <td>
                 <a href="{{ route('addmissionOn.edit',$item->id) }}" class="btn btn-sm btn-success" title="Edit">
         ...
         <a  class="btn btn-sm btn-danger" title="Delete" onclick="if(confirm('Are you sure to delete')){
            ...
            event.preventDefault();
                            ...
                            <form action="{{ route('addmissionOn.destroy',$item->id)}}" id="delete-form-{{ $item->id }}" method="POST">
                                @csrf
                                @method("DELETE")
                            </form>
