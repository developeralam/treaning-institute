...
@section('page-header')
    <i class="fa fa-list"></i> News & Update
@stop
@section('css')
...
@section('content')
    <div class="page-header">
        ...
        <a class="btn btn-xs btn-info" href="{{ route('newsUpdate.create') }}" role="button" class="blue" data-toggle="modal" style="float: right; margin: 0 2px;"> <i class="fa fa-plus"></i> Add New</a>
        ...
        <div class="col-xs-12">
            ...
            <div class="table-responsive" style="border: 1px #cdd9e8 solid;">
                <table id="dynamic-table" class="table table-striped table-bordered table-hover" >
                    ...
                    <tbody>
                         @foreach($newsUpdates as $key=>$newsUpdate)
                    <tr>
                       <td> {{ $key +1 }} </td>
                            <td> {{ ucfirst($newsUpdate->title) }} </td>
                           
                          
  {{--    <td>
         @if(!($newsUpdate->image == 'no'))
         
      <img class="img-responsive img-thumbnail" src="{{ asset('uploads/newsUpdate/'.$newsUpdate->image) }}"  style="height: 50px; weight: 30px;" alt=""> 

         @else
        <img class="img-responsive img-thumbnail" src="{{ asset('uploads/no.png') }}"  style="height: 50px; weight: 30px;" alt=""> 
      @endif
  </td> --}}
                           

             <td>
                 <a href="{{ route('newsUpdate.edit',$newsUpdate->id) }}" class="btn btn-sm btn-success" title="Edit">
                                <i class="fa fa-edit"></i>
                            </a> 
         <a  class="btn btn-sm btn-danger" title="Delete" onclick="if(confirm('Are you sure to delete')){
            event.preventDefault();
            document.getElementById('delete-form-{{ $newsUpdate->id }}').submit();       
        }else {
            event.preventDefault();
        }"><span class="glyphicon glyphicon-trash"></span></a>
                            <form action="{{ route('newsUpdate.destroy',$newsUpdate->id)}}" id="delete-form-{{ $newsUpdate->id }}" method="POST">
                                @csrf
                                @method("DELETE")
                            </form>
