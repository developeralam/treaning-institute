...
@section('content') 
 <div class="page-header"> 
        ...
        <a class="btn btn-xs btn-info" href="{{ route('expense.create')}}" role="button" class="blue" data-toggle="modal" style="float: right; margin: 0 2px;"> <i class="fa fa-plus"></i> Add New</a>
        ...
        <div class="col-xs-12">
            ...
            <div class="table-responsive" style="border: 1px #cdd9e8 solid;">
                <table id="dynamic-table" class="table table-striped table-bordered table-hover" >
                    ...
                    <tbody>
                         @foreach(App\Expense::getAllExpence() as $expence)
                    <tr>
                        <td>{{ $loop->index+1 }}</td>
                        <td>{{ $expence->party->name }}</td>
                    ...
                    <td>
                    <a href="{{url('expense-view')}}/{{$expence->id}}" class="btn btn-sm btn-success"><i class="fa fa-eye"></i></a>
                    <a  class="btn btn-sm btn-danger" title="Delete" onclick="if(confirm('Are you sure to delete')){
                    event.preventDefault();
                    document.getElementById('delete-form-{{ $expence->id }}').submit();       
                    }else {
                    event.preventDefault();
                    }"><span class="glyphicon glyphicon-trash"></span></a>
                    <form action="{{ route('expense.destroy',$expence->id)}}" id="delete-form-{{ $expence->id }}" method="POST">
                    @csrf
                    @method("DELETE")
                    </form>
