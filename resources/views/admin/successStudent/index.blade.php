...
@section('title','Success Student')
@section('page-header')
    <i class="fa fa-list"></i> Success Student
@stop
@section('css')
...
@section('content')
    ...
    <div class="page-header">
        ...
        <a class="btn btn-xs btn-info" href="{{ route('successStudent.create') }}" role="button" class="blue" data-toggle="modal" style="float: right; margin: 0 2px;"> <i class="fa fa-plus"></i> Add New</a>
                ...
                <table id="dynamic-table" class="table table-striped table-bordered table-hover" >
                    ...
                    <tr>
                       <td> {{ $key +1 }} </td>
                            ...
                            <td> {{ ucfirst($item->post) }} </td>
                          
     <td>
         @if(!($item->image == 'no'))
         
      <img class="img-responsive img-thumbnail" src="{{ asset('uploads/successstudent/'.$item->image) }}"  style="height: 50px; width: 50px;" alt=""> 

         @else
        <img class="img-responsive img-thumbnail" src="{{ asset('uploads/no.png') }}"  style="height: 50px; width: 50px;" alt=""> 
      @endif
  </td>
                           

             <td>
                 <a href="{{ route('successStudent.edit',$item->id) }}" class="btn btn-sm btn-success" title="Edit">
         ...
         <a  class="btn btn-sm btn-danger" title="Delete" onclick="if(confirm('Are you sure to delete')){
            ...
            event.preventDefault();
                            ...
                            <form action="{{ route('successStudent.destroy',$item->id)}}" id="delete-form-{{ $item->id }}" method="POST">
                                @csrf
                                @method("DELETE")
                            </form>
